 ...
 if(!empty($_POST)) {
  ...
  if(empty($err_msg)) {
   ...
   try{
    $dbh = dbConnect();
    if($edit_flg){
     $sql = 'UPDATE topic SET word = :word, mean = :mean, cat_id = :category, parts_id = :parts, text = :text, trans = :trans, pic = :pic WHERE user_id = :u_id, topic_id = :t_id';
     $data = array(':word' => $word, ':mean' => $mean, ':category' => $category, ':parts' => $parts, ':text' => $text, ':trans' => $trans, ':pic' => $pic, ':u_id' => $_SESSION['user_id'], ':p_id' => $p_id);
    } else {
...
debug('>>>>>画面表示処理終了');
 ...
 $siteTitle = 'ぴくたん | '(!$edit_flg) ? '新規単語登録' : '単語編集';
 require('head.php');
 require('header.php');
?>
   ...
   ?>
    ...
    <div class="new-post__inner">
     <label class="<?php if(!empty($err_msg['word'])) echo 'err'; ?>">
      ...
      <input type="text" name="word" value="<?php echo getFormData('word'); ?>">
     </label>
    </div>
    ...
    <div class="new-post__inner">
     <label class="<?php if(!empty($err_msg['mean'])) echo 'err'; ?>">
      ...
      <input type="text" name="mean" value="<?php echo getFormData('mean'); ?>">
     </label>
    </div>
    ...
    <div class="new-post__inner">
     <label class="<?php if(!empty($err_msg['text'])) echo 'err'; ?>">
      ...
      <input type="text" name="text" value="<?php echo getFormData('text'); ?>">
     </label>
    </div>
    ...
    <div class="new-post__inner">
     <label class="<?php if(!empty($err_msg['trans'])) echo 'err'; ?>">
      ...
      <input type="text" name="trans" value="<?php echo getFormData('trans'); ?>">
     </label>
    </div>
    ...
    <div class="new-post__inner">
     <label class="<?php if(!empty($err_msg['pic'])) echo 'err'; ?>">
      ...
      <input type="file" name="pic" class="new-post__input">
      <img src="<?php echo getFormData('pic'); ?>" class="new-post__prev">
     </label>
    </div>
